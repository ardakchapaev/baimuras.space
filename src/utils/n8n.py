        ...
        try:
            ...
            if current_app:
                current_app.logger.error(
                    f'Unexpected error in N8N request: {str(e)}')
            return None
        ...
        try:
            ...
            if current_app:
                current_app.logger.info(
                    f'Successfully sent webhook to n8n: {data.get("event_type", "unknown")}')
            return True

        except requests.exceptions.RequestException as e:
            if current_app:
                current_app.logger.error(
                    f'Failed to send webhook to n8n: {str(e)}')
            return False
        except Exception as e:
            if current_app:
                current_app.logger.error(
                    f'Unexpected error sending webhook to n8n: {str(e)}')
            return False
...
def send_to_n8n(data: Dict[str, Any],
                webhook_url: Optional[str] = None) -> bool:
    """
    Отправка данных в n8n webhook
...
def send_project_update_to_automation(
        project_data: Dict[str, Any], update_type: str) -> bool:
    """
...
def trigger_analytics_automation(
        analytics_type: str, parameters: Dict[str, Any] = None) -> bool:
    """

...
class N8NClient:
    ...
    def send_webhook(
        self, data: Dict[str, Any], webhook_url: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            response = requests.post(
                url, data=payload_str, headers=headers, timeout=self.timeout
            )
            response.raise_for_status()
...
def send_project_update_to_automation(
    project_data: Dict[str, Any], update_type: str
) -> bool:
    """
...
def trigger_analytics_automation(
    analytics_type: str, parameters: Dict[str, Any] = None
) -> bool:
    """

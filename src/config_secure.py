...
from .secret_manager import SecretManager, get_development_config, get_production_config, get_testing_config
...
class BaseConfig:
    ...
    def _setup_logging(self):
        ...
        log_level = logging.DEBUG if self.secret_manager.get_secret(
        ...
        logging.basicConfig(
            ...
            handlers=[
                ...
                logging.FileHandler('logs/baimuras.log') if os.path.exists('logs') else logging.NullHandler()
            ]
        )

        self.logger = logging.getLogger(__name__)
        self.logger.info(
            f"Инициализация конфигурации для среды: {self.environment}")
    ...
    def _load_config(self):
        ...
        self.DEBUG = self.secret_manager.get_secret(
            'DEBUG', 'False').lower() == 'true'
        self.TESTING = self.secret_manager.get_secret(
        ...
        self.MAIL_PORT = int(
            self.secret_manager.get_secret(
                'MAIL_PORT', '587'))
        self.MAIL_USE_TLS = self.secret_manager.get_secret(
        ...
        # Интеграции
        self.N8N_WEBHOOK_URL = self.secret_manager.get_secret(
            'N8N_WEBHOOK_URL')

...
class SecurityMiddleware:
    ...
    def init_app(self, app):
        ...
        self.max_requests_per_minute = app.config.get("MAX_REQUESTS_PER_MINUTE", 60)
        self.max_requests_per_hour = app.config.get("MAX_REQUESTS_PER_HOUR", 1000)
    ...
    def before_request(self):
        ...
        if self.is_suspicious_request():
            logger.warning("Suspicious request from %s: %s", client_ip, request.url)
...
def require_signature(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not hmac.compare_digest(signature, expected_signature):
            logger.warning("Invalid signature from %s", g.get('client_ip', 'unknown'))

...
from ..models import db, User, Lead, Project, Role
...
def update_user(user_id: int):
    ...
    try:
        ...
        if 'role' in data:
            ...
            if not role:
                return jsonify(
                    {'error': f'Role {data["role"]} not found'}), 400
            user.role = role
...
def create_lead():
    ...
    try:
        ...
        if not name or not email or not service_type:
            return jsonify(
                {'error': 'Name, email and service_type are required'}), 400
...
def convert_lead_to_project(lead_id: int):
    ...
    try:
        ...
        if not project_name or not description:
            return jsonify(
                {'error': 'Project name and description are required'}), 400
        ...
        current_app.logger.info(
            f'Lead {lead_id} converted to project {project.id}')

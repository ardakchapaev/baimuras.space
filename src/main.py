...
from flask import (  # pylint: disable=wrong-import-position
    Flask,
    send_from_directory,
    session,
)
...
from flask_limiter.util import (
    get_remote_address,  # pylint: disable=wrong-import-position
)
...
from src.content import FOOTER, NAVIGATION  # pylint: disable=wrong-import-position
...
from src.security_middleware import (  # pylint: disable=wrong-import-position
    RequestLogger,
    SecurityMiddleware,
)
...
def create_app(config_name: str = None) -> Flask:
    ...
    try:
        ...
        for key, value in vars(config_obj).items():
            if not key.startswith("_") and key != "secret_manager" and key != "logger":
        ...
        logger.info("Configuration loaded successfully for environment: %s", config_name)
    ...
    limiter = Limiter(
        ...
        default_limits=[application.config.get("RATELIMIT_DEFAULT", "100 per hour")],
    )
    ...
    if not talisman_config:
        ...
        Talisman(application, content_security_policy=csp, force_https=force_https)
    else:

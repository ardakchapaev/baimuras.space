...
class User(UserMixin, db.Model):
    ...
    consultations = db.relationship(
        'ConsultationRequest', backref='client', lazy=True)
...
class Lead(db.Model):
    ...
    assigned_to = db.relationship(
        'User', foreign_keys=[assigned_to_id], backref='assigned_leads')
...
class Project(db.Model):
    ...
    lead_id = db.Column(db.Integer, db.ForeignKey(
        'lead.id'))  # Если конвертирован из лида
    status = db.Column(db.Enum(ProjectStatus),
                       default=ProjectStatus.CONSULTATION)
    furniture_type = db.Column(db.Enum(FurnitureType), nullable=False)
    ...
    services = db.relationship(
        'Service', secondary=project_services, backref='projects')
    ...
    project_files = db.relationship(
        'ProjectFile', backref='project', lazy=True)
...
class ConsultationRequest(db.Model):
    ...
    phone = db.Column(db.String(20), nullable=False)
    furniture_type = db.Column(db.Enum(FurnitureType), nullable=False)
    # home_visit, office_visit, online
    ...
    consultant = db.relationship(
        'User', foreign_keys=[consultant_id], backref='consultations_assigned')
...
class ProjectFile(db.Model):
    ...
    # design, photo, document, specification

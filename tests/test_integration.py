...
from src.models import Lead, Role, User, db
from src.utils.jwt_utils import generate_tokens
from src.utils.n8n import send_lead_to_automation, send_to_n8n
...
def app():
    ...
    with app.app_context():
        ...
        test_user = User(email="test@baimuras.space", name="Test User", role=admin_role)
...
class TestAPIv1Integration:
    ...
    def test_auth_login(self, client, app):
        """Тест аутентификации"""
        login_data = {"email": "test@baimuras.space", "password": "testpass"}
    ...
    def test_convert_lead_to_project(self, client, auth_headers, app):
        ...
        with app.app_context():
            ...
            with patch(
                "src.utils.n8n.send_project_update_to_automation"
            ) as mock_automation:
                mock_automation.return_value = True
...
class TestN8NIntegration:
    ...
    def test_send_to_n8n_success(self, mock_post):
        ...
        test_data = {"event_type": "test", "data": {"test": "value"}}
...
class TestCeleryIntegration:
    ...
    def test_process_new_lead_event(self, mock_notification):
        ...
        payload = {"lead": {"id": 1, "name": "Test Lead", "email": "test@example.com"}}

        with patch(
            "automation.tasks.notification_tasks.assign_lead_to_manager"
        ) as mock_assign, patch(
            "automation.tasks.notification_tasks.schedule_lead_followup"
        ) as mock_followup:
...
class TestErrorHandling:
    ...
    def test_invalid_json_webhook(self, client):
        ...
        response = client.post(
            "/api/webhooks/n8n", data="invalid json", content_type="application/json"
        )

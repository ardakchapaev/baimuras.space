...
def setup_admin_panel(app: Flask, db):
    ...
    admin.add_view(SecureModelView(
        OrderItem, db.session, name='Позиции заказов'))
    admin.add_view(SecureModelView(ConsultationRequest,
                   db.session, name='Консультации'))
    ...
    admin.add_view(SecureModelView(
        ProjectFile, db.session, name='Файлы проектов'))

...
from automation.tasks.email_tasks import send_project_status_update
...
def check_overdue_measurements():
    ...
    try:
        ...
        logger.info("Checking for overdue measurements...")
...
def send_daily_measurement_reminders():
    ...
    try:
        ...
        return {"status": "success", "reminders_sent": measurements_count}
...
def notify_lead_assignment(lead_id: int, manager_id: int):
    ...
    try:
        ...
        message = (
            f"Вам назначен новый лид #{lead_id}. Пожалуйста, свяжитесь с клиентом."
        )
        ...
        logger.info(f"Lead {lead_id} assigned to manager {manager_id}")
...
def project_milestone_notification(project_id: int, milestone: str):
    ...
    try:
        ...
        logger.info(f"Project {project_id} milestone: {message}")

        return {"status": "success", "project_id": project_id, "milestone": milestone}
...
def send_weekly_digest():
    ...
    try:
        ...
        stats = {
            # Lead.query.filter(Lead.created_at >= week_ago).count()
            "new_leads": 0,
            # Project.query.filter(Project.created_at >= week_ago).count()
            "new_projects": 0,
            # Project.query.filter(Project.status == 'completed', Project.updated_at >= week_ago).count()
            "completed_projects": 0,
            # Measurement.query.filter(Measurement.scheduled_date >= datetime.now()).count()
            "measurements_scheduled": 0,
        }

        logger.info(f"Weekly digest prepared: {stats}")
